    ...
    <footer className="mt-16 border-t border-white/10 bg-black text-white">
      <div className="mx-auto max-w-7xl px-4 py-12">
        <div className="grid gap-10 md:grid-cols-3">
          ...
          <div className="flex items-start gap-4">
            ...
            />
            <div>
              <p className="text-2xl font-semibold leading-tight" style={{color: GOLD}}>
                SO Spa & Hamam
              </p>
              <p className="mt-2 max-w-xs text-white/80">{t("tagline")}</p>
              <p className="mt-3 text-sm text-white/60">{t("address")}</p>
              <a href="tel:+905306893420" className="mt-1 inline-block text-sm text-white/80 hover:opacity-80">
                {t("phone")}: +90 530 689 34 20
              </a>
            </div>
          </div>

          {/* ORTA: Navigasyon (dikey) */}
          <nav className="flex items-center md:justify-center">
            <ul className="flex flex-col gap-3 text-lg">
              {nav.map((n) => (
                <li key={n.href}>
                  <Link href={n.href} locale={locale} className="hover:opacity-90" style={{transition:"opacity .2s"}}>
                    {n.label}
                  </Link>
                </li>
              ))}
      ...
      <div className="border-t border-white/10" style={{backgroundColor: TOP_BG}}>
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-3 px-4 py-4 text-sm text-white/70 md:flex-row">
          <span>© {new Date().getFullYear()} SO Spa & Hamam. {t("rights")}</span>
        </div>
      </div>
    </footer>
  );
